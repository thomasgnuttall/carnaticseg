...
from src.tools import compute_novelty_spectrum, get_loudness, interpolate_below_length
...
distances_path = cpath('data', 'distances', 'svara', f'{track}', f'{svara}.csv')
try:
...
with open(distances_path,'a') as file:
    ...
    for i in tqdm.tqdm(all_ix):
        for j in all_ix:
            ...
            path, dtw_val = dtw(pat1, pat2, radius=round(l_longest*0.1))
            l = len(path)
            dtw_norm = dtw_val/l
...
metrics = []
all_ts = list(np.arange(2, 10))
for t in tqdm.tqdm(all_ts):
    clustering = fcluster(Z, t=t, criterion='maxclust')
...
plot_path = cpath('plots', f'{track}', 'hierarchical_svara_clustering_DHA.png')
...
t = 4
clustering = fcluster(Z, t=t, criterion='maxclust', R=None, monocrit=None)
